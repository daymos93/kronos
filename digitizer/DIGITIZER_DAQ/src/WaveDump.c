...
void setRunFile(char *msg) {
    ...
    FILE *fptr;
    if((fptr = fopen("/home/kronos/kronos/run/run", "w")) != NULL) {
...
void readRunFile(char *runfile) {
    ...
    FILE *fptr;
    if((fptr = fopen("/home/kronos/kronos/run/run", "r")) != NULL) {
...
int main(int argc, char *argv[])
{
    ...
    sprintf(log, "/home/kronos/kronos/data/%06d/log.txt", runid); // remote log file
    sprintf(outdir, "/home/kronos/kronos/data/%06d/HV%d_DIGITIZER/", runid, hvpoint); // destination dir
    ...
    f_ini = fopen("/home/kronos/kronos/run/cfg_digitizer_daq.ini", "r");
    if (f_ini == NULL ) {
...
InterruptTimeout:
        ...
        if (ElapsedTime > 1000) {
            if (Nb == 0)
                ...
                sprintf(msg, "Reading at %.2f MB/s (Trg Rate: %.2f Hz), event count = %d (%.0f%)", (float)Nb/((float)ElapsedTime*1048.576f), (float)Ne*1000.0f/(float)ElapsedTime, totCounter, (float)100*totCounter/maxtrig);
                logEntry(msg, log);
            }
            nCycles= 0;
            Nb = 0;
